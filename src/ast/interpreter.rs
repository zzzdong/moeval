...
pub struct Interpreter {
    ...
    functions: HashMap<String, FunctionItem>,
}
    ...
    fn eval_expression(&mut self, expr: &Expression) -> Result<ValueRef, Error> {
        match expr {
            ...
            Expression::Call(CallExpression { func, args }) => {
                self.eval_call_expression(func, args)
            }
    ...
    fn eval_call_expression(
        ...
        match func {
            Expression::Identifier(ident) => {
                if let Some(mut func) = self.variables.get(ident.0.as_str()) {
                    let args = args
                        .iter()
                    ...
                    let ret = func
                        .get_mut()
                        .call(&args)
                        .map(|ret| ret.unwrap_or(Null.into()))?;
                    Ok(ValueRef::new(ret))
                } else {
                    match self.functions.get(ident.0.as_str()).cloned() {
                        Some(FunctionItem {
                            name,
                            params,
                            return_ty,
                            ...
                            self.variables.exit_scope();
                            return Ok(ValueRef::new(Null.into()));
                        }
                        None => {
                            return Err(RuntimeError::symbol_not_found(ident.0.as_str()).into());
                        }
                    }
                }
            }
